...
if (!Main\Loader::includeModule('crm')) {
    ...
    if ($action === 'field') {
        ...
        } elseif ($fieldID == "ASSIGNED_BY_ID_DEPARTMENT") {
            $result = Main\UI\Filter\FieldAdapter::adapt([
        ...
        foreach ($filter->getFields() as $field) {
            $filterArray = $field->toArray(array('lightweight' => true));
...
//                echo '</pre>';
            ...
            $result[] = Main\UI\Filter\FieldAdapter::adapt($filterArray);
        }
        $result[] = Main\UI\Filter\FieldAdapter::adapt([
            "id" => "ASSIGNED_BY_ID_DEPARTMENT",
            "name" => "Подразделение",
