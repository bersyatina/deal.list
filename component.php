...
if (!$bInternal)
{
	...
	if ($externalFilterId)
	{
		...
		{
			$filterArray = $field->toArray();
			if ($field->getId() == 'ASSIGNED_BY_ID') {
				$filterArray['params']['multiple'] = true;
			}
			$arResult['FILTER'][] = Main\UI\Filter\FieldAdapter::adapt($filterArray);
		}
		$arResult['FILTER'][] = Main\UI\Filter\FieldAdapter::adapt([
			"id" => "ASSIGNED_BY_ID_DEPARTMENT",
			"name" => "Подразделение",
...
if (key_exists('%ASSIGNED_BY_ID_DEPARTMENT', $arFilter))
{
	$userIds = [];
	$userResult = Main\UserTable::getList([
		'select' => ['ID'],
		'filter' => [
			'UF_DEPARTMENT' => $arFilter['%ASSIGNED_BY_ID_DEPARTMENT'],
		],
	]);
	...
	if(count($userIds) > 0) {
		if (key_exists("ASSIGNED_BY_ID", $arFilter))
		{
			//Определяем заполнено ли поле ASSIGNED_BY_ID одним значением или множеством?
		}
		else{
			$arFilter['ASSIGNED_BY_ID'] = $userIds;
		}
	}
}
